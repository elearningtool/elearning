  ...
  class d_orders {
    ...
    function getOutput() {
      ...
      $output = '<div class="panel panel-default">' .
                ...
                '      <th class="col-md-4">' . MODULE_ADMIN_DASHBOARD_ORDERS_TITLE . '</th>' .
                '      <th class="col-md-2">' . MODULE_ADMIN_DASHBOARD_ORDERS_TOTAL . '</th>' .
                '      <th class="col-md-2">' . MODULE_ADMIN_DASHBOARD_ORDERS_DATE . '</th>' .
                '      <th class="col-md-4 text-right">' . MODULE_ADMIN_DASHBOARD_ORDERS_ORDER_STATUS . '</th>' .
                '    </tr>';
      ...
      while ($orders = tep_db_fetch_array($orders_query)) {
        $output .= '    <tr>' .
                   '      <td class="col-md-4"><a href="' . tep_href_link(FILENAME_ORDERS, 'oID=' . (int)$orders['orders_id'] . '&action=edit') . '">' . tep_output_string_protected($orders['customers_name']) . '</a></td>' .
                   '      <td class="col-md-2">' . strip_tags($orders['order_total']) . '</td>' .
                   '      <td class="col-md-2">' . tep_date_short($orders['date_last_modified']) . '</td>' .
                   '      <td class="col-md-4 text-right">' . $orders['orders_status_name'] . '</td>' .
                   '    </tr>';
      }
